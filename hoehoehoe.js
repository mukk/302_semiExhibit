(function() {
	...
	if(!webgl_flg){
		...
		spot1.position.set(-160, 200, 90);
		spot1.target.position.set(-50,100,0);
		scene.add(spot1);

		var spot2 = new THREE.SpotLight(0xff0000);
		spot2.position.set(-20, -40, 100);
		spot2.target.position.set(20,10,0);
		scene.add(spot2);

		var spot3 = new THREE.SpotLight(0x00ff00);
		spot3.position.set(50, -50, 80);
		spot3.target.position.set(0,60,0);
		scene.add(spot3);
		...
		// }());
		for (var i = 0; i < 9; i++) {
			if(i==1)U[HALF_X][HALF_Y] = 5;U[HALF_X+20][HALF_Y-10] = 2;
			if(i==7)U[HALF_X+3][HALF_Y-5] = -10;
			if(i==8)U[HALF_X+8][HALF_Y-8] = 4;U[HALF_X+20][HALF_Y-10] = -10;
			renderer.render(scene,camera);
			updateWave();
		}
		renderer.render(scene,camera);
